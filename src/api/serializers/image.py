...
class StdImageSerializer(serializers.Serializer):
    ...
    def get_original(self, picture: StdImageFieldFile):
        return picture.url

    def get_thumbnail(self, picture: StdImageFieldFile):
        if hasattr(picture, "thumbnail"):
            return picture.thumbnail.url
        return None

    def get_medium(self, picture: StdImageFieldFile):
        if hasattr(picture, "medium"):
            return picture.medium.url
        return None

    def get_large(self, picture: StdImageFieldFile):
