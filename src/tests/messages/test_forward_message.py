...
def test__forward_message__success_case(api_client):
    ...
    response = api_client.post(reverse("message-forward"),
                               data={"forward_to": {"pk": chat_to_forward.pk},
                                     "message_ids": [{"pk": messages[0].pk}, {"pk": messages[1].pk},
                                                     {"pk": messages[2].pk}]}, format="json")
